...
def edit_multiline(default):
    info(default)
    info("[直接回车使用默认，输入 e 进入编辑]")
    choice = input("> ").strip().lower()
    if choice != "e":
        return default
    info("粘贴新提示词，完成后新行输入 END 回车：")
    lines = []
    while True:
        line = input()
        if line == "END":
...
def call_llm_stream(prompt):
    ...
    sys.stdout.write("\n")
    sys.stdout.flush()
    info(f"[LLM] stream call duration: {round(time.time() - start, 2)} s")
    return full
...
def run_phase1(srt_path, output_dir=None, interactive=True):
    ...
    if step1_path:
        with open(step1_path, "w", encoding="utf-8") as f:
            f.write(result)
        info(f"[Stage 1] 已保存: {step1_path}")
    info(f"[Stage 1] duration: {round(time.time() - phase_start, 2)} s")
...
def run_phase2(phase1_result, output_dir=None, interactive=True):
    ...
    if step2_path:
        with open(step2_path, "w", encoding="utf-8") as f:
            f.write(result)
        info(f"[Stage 2] 已保存: {step2_path}")
    info(f"[Stage 2] duration: {round(time.time() - phase_start, 2)} s")
...
def run_phase3(srt_path, script, output_dir=None):
    ...
    if intervals_path and os.path.exists(intervals_path):
        warn(f"[跳过] 已有缓存: {intervals_path}")
        with open(intervals_path, "r", encoding="utf-8") as f:
            return json.load(f)

    info("[Stage 3] 开始 AI 字幕匹配 ...")
    ...
    info(f"[Stage 3] 共匹配 {len(valid)} 个片段（{skipped} 个未匹配已跳过）")
    for i, item in enumerate(valid):
        start, end = item[0]
        text = str(item[1])
        info(f"{i + 1}. [{start} --> {end}] {text[:40]}")

    if intervals_path:
        with open(intervals_path, "w", encoding="utf-8") as f:
            json.dump(valid, f, ensure_ascii=False, indent=2)
        info(f"[Stage 3] 已保存: {intervals_path}")
    info(f"[Stage 3] duration: {round(time.time() - phase_start, 2)} s")
...
def run_phase4(video_path, keep_intervals, video_id):
    ...
    info("=" * 60)
    phase_start = time.time()
    info(f"[Stage 4] 开始剪辑，共 {len(keep_intervals)} 个片段 ...")
    output_path = cut_video_main(keep_intervals, video_path, video_id, "cli")
    info(f"[Stage 4] 视频已生成: {output_path}")
    info(f"[Stage 4] duration: {round(time.time() - phase_start, 2)} s")
    return output_path
...
def main():
    ...
    info(f"[Init] video_id={video_id}  output_dir={output_dir}  stage={args.stage}")
